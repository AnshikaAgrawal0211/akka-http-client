...
//    val responseFuture: Future[HttpResponse] = Http().singleRequest(HttpRequest(uri = "https://www.owler.com/distil_identify_cookie.html?httpReferrer=%2Fcompany%2Fknoldus&uid=3DDA8C31-0456-3C4A-8505-CD67EE2551C3", headers = Seq(cookieHeader,userAgent)))
      val responseFuture: Future[HttpResponse] = Http().singleRequest(HttpRequest(uri = "https://www.owler.com/company/knoldus").withHeaders(cookieHeader,userAgent))
    ...
    responseFuture
      .onComplete {
        case Success(res) => {
          ...
          println(entity)
          println(res)
           res.entity.dataBytes.map(_.utf8String).runForeach(data=> println(data))
          system.terminate()
        }
